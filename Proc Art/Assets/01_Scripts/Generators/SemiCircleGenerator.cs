...
public class SemiCircleGenerator : StructureGenerator {
    ...
    public IEnumerator Generate(SemiCircleConfig _config) {
        ...
        for(int y = 0; y < _config.brickAmount.y; y++) {
            ...
            for(int x = 0; x < _config.brickAmount.x; x++) {
                ...
                float theta = _config.rotationOffset + (x * brickSize.x + xOffset) * (Mathf.PI / _config.brickAmount.x);
    ...
    private float CalculateRadius(int _amountX, float _marginX) {
        float innerRadius = (_amountX * 2 * (brickSize.x + _marginX)) / (2 * Mathf.PI);
